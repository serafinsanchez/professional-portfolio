export const meta = {
  title: 'Live Class Poll - Real-time Educational Polling Platform',
...
# Live Class Poll - Real-time Educational Polling Platform
...
1. **Educator Creates Poll:** Navigate to homepage, click "Create Poll", select poll type (multiple choice/free text/song link), add question and options, configure timer settings
2. **Poll Sharing:** Automatic QR code generation, copy shareable link, display QR code on projector or share link via LMS
3. **Student Participation:** Scan QR code or click link, view question and options, submit vote or response, see real-time results (if enabled)
4. **Real-time Monitoring:** Instructor watches live results on share page, analyzes participation patterns, exports data for grading or analysis
5. **Post-Poll Analytics:** Access dashboard for historical data, export results to CSV, create follow-up polls based on feedback
6. **(Premium)** Email Collection: High-engagement users see email capture modal, unlock advanced dashboard features, receive poll management notifications
...
- **Input Validation:** Server-side sanitization of all user-generated content and poll responses
- **CORS Configuration:** Proper cross-origin resource sharing settings for secure API access
...
- **Next.js 15:** React framework with App Router for modern routing and server components
...
- **Framer Motion:** Smooth animations and micro-interactions for enhanced user experience

### Backend Technologies
- **Firebase Firestore:** Real-time NoSQL database with offline support and automatic scaling
- **Firebase Admin:** Server-side SDK for secure database operations and user management
- **Next.js API Routes:** Serverless functions for email processing and authentication
- **Node.js Runtime:** Modern JavaScript execution environment with full ES2022 support

### Development & Testing
- **Jest:** Unit testing framework with React Testing Library for component testing
- **Playwright:** End-to-end testing across multiple browsers with visual regression testing
- **ESLint:** Code quality enforcement with educational-focused linting rules
- **TypeScript Compiler:** Compile-time error detection and enhanced IDE support

### Additional Services
- **QR Code Generation:** Dynamic QR codes for instant poll access without manual URL sharing
- **CSV Export System:** Server-side data processing for spreadsheet-compatible downloads
- **Email Collection API:** Strategic engagement tracking with privacy-compliant data handling
- **Analytics Integration:** User behavior tracking for continuous product improvement
