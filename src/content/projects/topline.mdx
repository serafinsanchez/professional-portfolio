export const meta = {
  ...
  repo: 'https://github.com/serafinsanchez/toplineapp',
  demo: 'https://acapellas.app',
  cover: '/images/projects/topline.gif'
}

# Topline

A modern web application designed for musicians, producers, and audio professionals who need to extract individual stems (vocals, drums, bass, etc.) from audio files. The platform streamlines the process of uploading tracks, processing them in the cloud, and delivering high-quality separated audio components, all while managing user access, credits, and payments in a secure, scalable environment.
...
- Seamless Stripe integration for purchasing credits
...
1. **Sign Up / Log In:** User creates an account or logs in using secure authentication
2. **Upload Audio:** User uploads an audio file via a drag-and-drop interface
3. **Purchase Credits:** If needed, user purchases credits through Stripe
4. **Extract Stems:** User initiates stem extraction; credits are deducted automatically
5. **Download Results:** User receives and downloads the separated audio stems
6. **Profile & History:** User can view their profile, credit balance, and transaction history
7. **(Admin) Dashboard:** Admins access a dashboard to manage users and credits
...
- Custom CORS headers for API routes and secure HTTP headers
...
- **MOISES API** for stem extraction
...
**Key Results:**
- Professional-grade audio stem extraction accessible via web browser
- Streamlined credit system with transparent pricing and usage tracking
- Secure, scalable architecture supporting concurrent audio processing
- Modern, responsive interface optimized for musicians and producers
